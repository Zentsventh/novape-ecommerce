...
const CategoryIcon = ({ name }) => {
    const icons = {
        ...
        'Mundo Gamer': (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 2l4 10h-8z"/><path d="M12 22l-4-10h8z"/>
            </svg>
        ),
        'Audio': (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        'Smartwatches': (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <rect x="6" y="4" width="12" height="16" rx="3" /><line x1="9" y1="2" x2="15" y2="2" /><line x1="9" y1="22" x2="15" y2="22" /><polyline points="12 8 12 12 14 14" />
            </svg>
        ),
                    ...
                    {categorias.map((cat) => (
                        ...
                        >
                            <button
                                ...
                                aria-haspopup={cat.subcategorias && cat.subcategorias.length > 0 ? 'menu' : undefined}
                                aria-expanded={hoverId === cat.id ? 'true' : 'false'}
                                aria-controls={cat.subcategorias && cat.subcategorias.length > 0 ? `cat-submenu-${cat.id}` : undefined}
                            >
                            ...
                            {/* Mega Menu para subcategorías */}
                            {hoverId === cat.id && cat.subcategorias && cat.subcategorias.length > 0 && (
                                <div className="efe-cat-submenu" role="menu" id={`cat-submenu-${cat.id}`}>
