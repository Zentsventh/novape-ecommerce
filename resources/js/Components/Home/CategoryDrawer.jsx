...
export default function CategoryDrawer({ isOpen, onClose, categorias = [] }) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div className={`efe-cat-drawer ${isOpen ? 'is-open' : ''} ${isMobile && activeId ? 'show-right' : ''}`}>
                ...
                <div className="efe-cat-drawer-right">
                    {activeCat ? (
                        <>
                            <div className="efe-cat-drawer-sub-header" style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '20px', borderBottom: '2px solid #f1f5f9', paddingBottom: '15px' }}>
                                {isMobile && (
                                    <button 
                                        onClick={handleBack} 
                                        style={{ alignSelf: 'flex-start', background: 'transparent', border: 'none', color: '#64748b', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', padding: 0, fontWeight: 'bold' }}
                                    >
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                                    <h4 style={{ fontSize: '20px', fontWeight: '900', color: '#0f172a', textTransform: 'uppercase', position: 'relative', margin: 0, letterSpacing: '-0.5px' }}>
                                        {activeCat.nombre}
                                    ...
                                    <button 
                                        ...
                                        style={{ fontSize: '12px', fontWeight: 'bold', color: '#00B4FF', background: 'rgba(0,180,255,0.1)', padding: '6px 14px', borderRadius: '20px', border: 'none', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '6px' }} 
                                    >
                                        Ver todo
                                    </button>
                                </div>
                            ...
                            <div className="efe-cat-drawer-sub-list" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginTop: '10px', overflowY: 'auto', paddingBottom: '20px' }}>
                                {activeCat.subcategorias && activeCat.subcategorias.length > 0 ? (
                                    activeCat.subcategorias.map(sub => (
                                        <button 
                                            ...
                                            style={{ 
                                                ...
                                                transition: 'all 0.2s ease', wordBreak: 'break-word'
                                            }}
                                        >
                                    ...
                                    <div style={{ gridColumn: '1 / -1', background: '#f8fafc', borderRadius: '12px', padding: '30px', border: '1px dashed #cbd5e1', textAlign: 'center' }}>
                                        <div style={{ color: '#64748b', fontWeight: '500', fontSize: '13px' }}>Sin subcategorías</div>
                                    </div>
                                )}
                            </div>
                        </>
                    ) : (
                        <div className="efe-cat-drawer-empty" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#9ca3af', gap: '12px', fontSize: '14px', textAlign: 'center', padding: '40px' }}>
